...
def hash_stream(url, timeout=(4, 8)):
    ...
    head = _range(url, 0, CHUNK - 1, timeout)
    if head is None or len(head) < CHUNK:
        return "", 0
    tail = _range(url, size - CHUNK, size - 1, timeout)
...
def _range(url, start, end, timeout):
    response = http.get(url, retries=1, timeout=timeout, stream=True,
    ...
    try:
        ...
        _discard(response)
    # A server that ignores Range sends the whole file; take only what we need.
    if response.status_code == 200 and len(data) > CHUNK:
        return data[:CHUNK] if start == 0 else data[-CHUNK:]
    return data
